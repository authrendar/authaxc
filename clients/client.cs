    ...
    class Program
    {
        ...
        private static async Task<bool> RegisterUser(string username, string password, string licenseKey)
        {
            ...
            try
            {
                ...
                {
                    string expiry = ExtractJsonField(responseBody, "expires_at");
                    string subLevel = ExtractJsonField(responseBody, "subscription_level");
                    Console.WriteLine("[+] Registration Successful!");
                    Console.WriteLine($"[+] License Expiry: {(string.IsNullOrEmpty(expiry) ? "Lifetime" : expiry)}");
                    if (!string.IsNullOrEmpty(subLevel)) Console.WriteLine($"[+] Subscription Level: {subLevel}");
                    return true;
                }
                else
                {
        ...
        private static async Task<bool> LoginUser(string username, string password)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Console.WriteLine($"[+] License Expiry Date: {(string.IsNullOrEmpty(expiry) ? "Lifetime" : expiry)}");
                    if (!string.IsNullOrEmpty(subLevel)) Console.WriteLine($"[+] Subscription Level: {subLevel}");
                    return true;
                }
                else
        ...
        {
            return ExtractJsonField(json, "detail");
        }
        ...
        private static string ExtractJsonField(string json, string fieldName)
        {
            try
            {
                ...
                // Try number/bool matching without quotes
                searchToken = $"\"{fieldName}\":";
                index = json.IndexOf(searchToken);
                if (index != -1)
                {
                    int start = index + searchToken.Length;
