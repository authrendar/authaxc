...
void Register() {
    ...
    if (status == "success") {
        std::string exp = GetJSONValue(response, "expires_at");
        std::string sub = GetJSONValue(response, "subscription_level");
        std::cout << "[+] Registration Successful!\n";
        std::cout << "[+] Expiry Date: " << (exp.empty() ? "Lifetime" : exp) << "\n";
        if (!sub.empty()) std::cout << "[+] Subscription Level: " << sub << "\n";
    } else {
...
bool Login() {
    ...
    if (status == "success") {
        ...
        std::cout << "[+] License Expiry Date: " << (exp.empty() ? "Lifetime" : exp) << "\n";
        if (!sub.empty()) std::cout << "[+] Subscription Level: " << sub << "\n";
        return true;
    } else {
        std::string detail = GetJSONValue(response, "detail");
